...
func TestCreateGeneration_NewReplicaSet(t *testing.T) {
	t.Run("creates generation with UUID and correct fields", func(t *testing.T) {
		s := New()
...
func TestCreateGeneration_UpdatesActiveGeneration(t *testing.T) {
	t.Run("second generation becomes active", func(t *testing.T) {
		s := New()
		ctx := context.Background()
		replicaSet := orchestrator.ReplicaSetName("main-projections")
...
func TestRegisterWorker_PendingState(t *testing.T) {
	t.Run("worker registered in pending state with unassigned partition", func(t *testing.T) {
		s := New()
		ctx := context.Background()
		replicaSet := orchestrator.ReplicaSetName("main-projections")
...
func TestAssignPartition(t *testing.T) {
	t.Run("assigns partition to worker", func(t *testing.T) {
		s := New()
		ctx := context.Background()
		replicaSet := orchestrator.ReplicaSetName("main-projections")
...
func TestUpdateWorkerState_Transitions(t *testing.T) {
	testCases := []struct {
		name      string
		fromState orchestrator.WorkerState
		toState   orchestrator.WorkerState
	}{
		{
			name:      "Pending to Ready",
			fromState: orchestrator.WorkerStatePending,
			toState:   orchestrator.WorkerStateReady,
		},
		{
			name:      "Ready to Running",
			fromState: orchestrator.WorkerStateReady,
			toState:   orchestrator.WorkerStateRunning,
		},
		{
			name:      "Running to Stopping",
			fromState: orchestrator.WorkerStateRunning,
			toState:   orchestrator.WorkerStateStopping,
		},
		{
			name:      "Stopping to Stopped",
			fromState: orchestrator.WorkerStateStopping,
			toState:   orchestrator.WorkerStateStopped,
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Set initial state
			if tc.fromState != orchestrator.WorkerStatePending {
				err = s.UpdateWorkerState(ctx, worker.ID, tc.fromState)
				require.NoError(t, err)
			}

			// Transition to target state
			err = s.UpdateWorkerState(ctx, worker.ID, tc.toState)
			require.NoError(t, err)

			// Verify state change
			updatedWorker, err := s.GetWorker(ctx, worker.ID)
			require.NoError(t, err)
			assert.Equal(t, tc.toState, updatedWorker.State)
		})
	}
}

func TestHeartbeat_UpdatesTimestamp(t *testing.T) {
	t.Run("heartbeat updates LastHeartbeat timestamp", func(t *testing.T) {
		s := New()
		ctx := context.Background()
		replicaSet := orchestrator.ReplicaSetName("main-projections")
...
func TestGetWorkersByGeneration(t *testing.T) {
	t.Run("returns only workers for specified generation", func(t *testing.T) {
		s := New()
		ctx := context.Background()
		replicaSet := orchestrator.ReplicaSetName("main-projections")
...
func TestGetActiveWorkers_ExcludesStopped(t *testing.T) {
	t.Run("returns only non-stopped workers", func(t *testing.T) {
		s := New()
		ctx := context.Background()
		replicaSet := orchestrator.ReplicaSetName("main-projections")
...
func TestGetPendingWorkers(t *testing.T) {
	t.Run("returns only pending workers", func(t *testing.T) {
		s := New()
		ctx := context.Background()
		replicaSet := orchestrator.ReplicaSetName("main-projections")
...
func TestMarkWorkerDead(t *testing.T) {
	t.Run("marks worker as stopped", func(t *testing.T) {
		s := New()
		ctx := context.Background()
		replicaSet := orchestrator.ReplicaSetName("main-projections")
...
func TestConcurrentAccess(t *testing.T) {
	t.Run("handles concurrent operations without race conditions", func(t *testing.T) {
		s := New()
		ctx := context.Background()
		replicaSet := orchestrator.ReplicaSetName("main-projections")
